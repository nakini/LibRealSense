...
int main(int argc, char * argv[]) try
{
    ...
    std::stringstream savepath("/home/vigir/realsense_data/");
    ...
    while (app)
    {
	 ...
	 // We can only save video frames as pngs, so we skip the rest
		if (auto vf = f.as<rs2::video_frame>())
		{
		   ...
		   file_name << savepath.str() << cam_id << "/rs-" << ctr << "-"  << 	vf.get_profile().stream_name() << "_raw.txt";
		   // stbi_write_png(png_file.str().c_str(), vf.get_width(), vf.get_height(),
		    ...
		    if(fptr == NULL){
		    	std::cout << "Could not open file" << png_file.str() << std::endl;
			fclose(fptr); 
			return EXIT_SUCCESS;
